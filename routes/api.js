...
const requestWithdraw = async (req, res) => {//출금신청
    try {
        ...
        let request_withdraw_history = await dbQueryList(`SELECT * FROM log_star_table WHERE type=4 AND user_pk=${decode?.pk} AND price < 0 AND status != -1 AND date LIKE '%${returnMoment().substring(0,10)}%'`);
